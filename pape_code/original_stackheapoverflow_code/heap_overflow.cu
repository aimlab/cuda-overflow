...
int main()
{
	...
	unsigned long *dev_input;
	  int *dev_admin;
	  for(int i=0;i<4;i++)
	 	 	input[i]=0x1c0000001c0;
	 	for(int i=4;i<20;i++)
	 		input[i]=0x50263f920;
	CUDA_CHECK_RETURN(cudaMalloc((void**)&dev_hashes,N*sizeof(unsigned long)));
	CUDA_CHECK_RETURN(cudaMalloc((void**)&dev_input,20*sizeof(unsigned long)));
	CUDA_CHECK_RETURN(cudaMalloc((void**)&dev_admin,sizeof(int)));
	CUDA_CHECK_RETURN(cudaMemcpy(dev_input,input,20*sizeof(unsigned long),cudaMemcpyHostToDevice));
	CUDA_CHECK_RETURN(cudaMemcpy(dev_admin,&admin,sizeof( int),cudaMemcpyHostToDevice));
   // cout<<"start!"<<endl;

//0x50263f920
//0x1c0000001c0
	test_kernel<<<1,1>>>(dev_hashes,dev_input, len,dev_admin);
