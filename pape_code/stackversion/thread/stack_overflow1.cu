...
// dummy9() simulate malicious function
#define BUF_LEN 16
#define N 2
...
__device__  unsigned long __noinline__ unsafe(unsigned int *input,int len)
{
	...
	{
		hash=((hash<<5)+hash)+buf[i];//compute hash value of input
		printf("%d\n", hash%8 );
	}
...
__global__ void test_kernel(unsigned long *hashes,unsigned int *input,int len,int admin)
{
	...
	if(admin)//to avoid optimization of compiler
	{	my_hash=dummy9();
		//my_hash=dummy8();
//	printf("%p\n",&idx);
...
int main()
{
	...
	m=0x27000000270;//address of dummy9() and depend on your computer
	//m=0x450;
	...
	test_kernel<<<1,2>>>(dev_hashes,dev_input,len,admin);
